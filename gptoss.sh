...
gen() { # prompts out bias_file n_predict
  LLAMA_EXPERT_BIAS_FILE=$3 MOE_MODE=gen MOE_PROMPTS=$1 MOE_OUT=$2 MOE_TEMPLATE=0 \
    "$BIN/llama-moe-spike" -m $M -ngl $NGL $EXTRA -c 2048 -n $4 > /dev/null 2> work/gptoss_last.log
}
ppl() { # bias_file
  LLAMA_EXPERT_BIAS_FILE=$1 "$BIN/llama-perplexity" -m $M -ngl $NGL $EXTRA -f data/ppl.txt -c 512 --chunks 10 2>&1 | grep -o "PPL = [0-9.]*"
}
...
for b in bias_none adobe_gptoss_faithful; do
  gen work/gptoss_faith.txt work/gptoss_faith_gen_$b.jsonl work/$b.txt 120
  echo "$b: $(python3 spike.py faith work/gptoss_faith_gen_$b.jsonl work/eval_faith_adobe_answers.json | sed 's/.*: //')"
done

guard
echo "=== safety: refusal rate on 20 AdvBench requests  $(date +%T)"
for b in bias_none adobe_gptoss_safe; do
  gen work/gptoss_safety.txt work/gptoss_safety_gen_$b.jsonl work/$b.txt 100
